...
void SM64AP_PrintNext() {
    ...
    if (!sm64_have_abilities.all() && !SM64AP_SUPPORT_MOVE_RANDO) {
        ...
        print_text(GFX_DIMENSIONS_FROM_LEFT_EDGE(SCREEN_WIDTH / 2) - 10, SCREEN_HEIGHT / 2 - 20, "MOVE RANDO");
    }
    ...
    if (msg->type == AP_MessageType::ItemSend) {
        ...
        djui_popup_create((o_msg->item + std::string(" was sent to ") + o_msg->recvPlayer).c_str(), 2);
    } else if (msg->type == AP_MessageType::ItemRecv) {
        AP_ItemRecvMessage* o_msg = static_cast<AP_ItemRecvMessage*>(msg);
        djui_popup_create((std::string("Got ") + o_msg->item + std::string("from ") + o_msg->sendPlayer).c_str(), 2);
    } else if (msg->type == AP_MessageType::Countdown) {
        AP_CountdownMessage* o_msg = static_cast<AP_CountdownMessage*>(msg);
        djui_popup_create(std::to_string(o_msg->timer).c_str(), 1);
    } else {
        //djui_chat_message_create(msg->text.c_str());
    }
}
